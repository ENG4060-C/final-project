...
WEBSOCKET_URL = "ws://localhost:8001/ws/telemetry?client=frontend"
LABEL_SETS = [
    ["person"],
    ["laptop", "chair"]
]
    ...
    async def display_frame(self, data):
        ...
        try:
            ...
            self.draw_telemetry_overlay(
                frame, ultrasonic, motors, detections, labels, timestamp
            )
    ...
    def draw_telemetry_overlay(self, frame, ultrasonic, motors, detections, labels, timestamp):
        ...
        if detections:
            ...
            for i, det in enumerate(detections[:5]):  # Show max 5 detections
                ...
                # Get text size
                (text_width, text_height), baseline = cv2.getTextSize(
                    det_text, font, font_scale, thickness
                )
